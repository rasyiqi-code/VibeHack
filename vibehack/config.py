...
class Config:
    ...
    def load(self):
        ...
        self.MODEL = os.getenv(
            "VH_MODEL", "openrouter/google/gemini-2.0-flash-exp:free"
        )
        ...
        # Provider Keys (Env Var > Keyring > fallback to VH_API_KEY)
        self.OPENROUTER_KEY = (
            os.getenv("OPENROUTER_API_KEY") or get_api_key("openrouter") or ""
        )
        self.GOOGLE_KEY = (
            os.getenv("GEMINI_API_KEY")
            or os.getenv("GOOGLE_API_KEY")
            or get_api_key("google")
            or ""
        )
        ...
        self.ANTHROPIC_KEY = (
            os.getenv("ANTHROPIC_API_KEY") or get_api_key("anthropic") or ""
        )
        self.OPENAI_KEY = os.getenv("OPENAI_API_KEY") or get_api_key("openai") or ""
        self.GITHUB_TOKEN = (
            os.getenv("GITHUB_TOKEN")
            or os.getenv("GH_TOKEN")
            or get_api_key("github")
            or ""
        )

        # Primary key mapping - use provider-specific key if VH_API_KEY not set
        self.API_KEY = (
            os.getenv("VH_API_KEY")
            or get_api_key("primary")
            or self._get_provider_key()
        )
        ...
        self.PRIMARY_DEFAULT = self.DEFAULTS.get(
            "primary_default", "openrouter/anthropic/claude-3.5-sonnet"
        )
