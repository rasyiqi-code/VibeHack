...
def install_docker_engine():
    ...
    console.print(
        "I can safely auto-install it via system apt-get (requires sudo password)."
    )
    ...
    try:
        ...
        if user:
            ...
            console.print(
                "[bold yellow]Note: You may need to restart your terminal or type 'newgrp docker' for group changes to take effect if permission denied errors occur.[/bold yellow]"
            )
        ...
        console.print(
            "[bold red]ERROR: sudo or apt-get not found. Only Debian/Ubuntu/Kali bases are supported for auto-install.[/bold red]"
        )
        sys.exit(1)
...
def pull_image_if_needed():
    ...
    with console.status(f"[dim]Checking docker image {IMAGE_NAME}...[/dim]"):
        try:
            ...
            if res.returncode != 0:
                console.print(
                    f"[dim]Pulling sandbox image: {IMAGE_NAME} (this may take a minute)[/dim]"
                )
...
def start_sandbox():
    """Initializes the docker sandbox environment with security hardening."""
    ...
    if not check_docker():
        ...
        if not check_docker():
            console.print(
                "[bold red]Docker still not detected after installation attempt. Exiting.[/bold red]"
            )
            import sys
    ...
    with console.status("[dim]Starting Hardened Ephemeral Sandbox...[/dim]"):
        # 1. Create a dedicated workspace if it doesn't exist
        workspace_dir = cfg.HOME / "workspace"
        workspace_dir.mkdir(parents=True, exist_ok=True)

        # 2. Create temporary directory for /tmp isolation
        tmp_dir = cfg.HOME / "sandbox_tmp"
        tmp_dir.mkdir(parents=True, exist_ok=True)

        # 3. Security Hardening:
        # - NO root .vibehack mount (protect memory.db, sessions)
        # - Read-only binaries
        # - Isolated tmp (cleared per-session)
        ...
        cmd = [
            ...
            "bridge",
            "-p",
            "127.0.0.1::30000",  # No host port binding
            # Mounts
            "-v",
            f"{cfg.BIN_DIR.absolute()}:/usr/local/bin:ro",
            "-v",
            f"{workspace_dir.absolute()}:/root/workspace:rw",
            "-v",
        ...
        try:
            ...
            console.print(
                "[dim]  Path: /root/workspace (RW), /usr/local/vibehack/bin (RO)[/dim]"
            )
        except subprocess.CalledProcessError as e:
            console.print(
                f"[bold red]Failed to start sandbox: {e.stderr.decode()}[/bold red]"
            )
            raise
...
def ensure_sandbox_running() -> bool:
    """
    Ensures sandbox is running, with auto-recovery on failure.
    ...
    if not check_docker():
        console.print("[bold red]Docker not available.[/bold red]")
        return False
    ...
    try:
        ...
        console.print(f"[bold red]Sandbox auto-recovery failed: {e}[/bold red]")
        return False
